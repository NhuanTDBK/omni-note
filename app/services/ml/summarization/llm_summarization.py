...
class ContentSummarizer(BaseAgent):
    ...
    def from_config(config: Config):
        client = AsyncClient(
            api_key=config.TEXT_AGENT_SUMMARIZATION_LLM_API_KEY,
            base_url=config.TEXT_AGENT_SUMMARIZATION_LLM_URL,
        )
        model_id = config.TEXT_AGENT_SUMMARIZATION_LLM_MODEL
        return ContentSummarizer(client, model_id)
    ...
    async def summarize_content(
        ...
        prompt = self.prompt_template.format(
            content=content, tone_template=tone_template
        )
